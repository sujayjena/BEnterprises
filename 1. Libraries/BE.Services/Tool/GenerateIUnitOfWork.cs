...
{
    public class GenerateIUnitOfWork
    {
        ...
        public void GenerateModelFiles()
        {
            try
            {
                ...
                string sModelClassFilesSaveInFolder = "UnitOfWork";
                string sProjectName = Assembly.GetCallingAssembly().GetName().Name + sFolderNameModel;

                string rootPath = AppDomain.CurrentDomain.BaseDirectory + sModelClassFilesSaveInFolder;
                ...
                string path = @"" + sPath + "IUnitOfWork.cs";
                //if (File.Exists(path))
                //{
                //    File.Delete(path);
                //    File.Create(path);
                ...
                using (StreamWriter sw = File.CreateText(path))
                {
                    ...
                    sw.WriteLine(@"    " + "public interface IUnitOfWork");
                    sw.WriteLine(@"    " + "{");

                    sw.WriteLine(@"        " + "void Save();");
                    sw.WriteLine(@"");
                    ...
                    foreach (DataRow item in dtAllTables.Rows)
                    {
                        ...
                        sw.WriteLine(@"        " + "IGenericRepBEnterprisesitory<" + TablesName + "> " + TablesName + "_RepBEnterprisesitory " + getSet);
                    }
                    sw.WriteLine(@"    " + "}");
                    sw.WriteLine(@"}");
                }
